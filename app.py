...
# ─────────────────────────────
# Admin Page (organized with a top "bar" using tabs)
# ─────────────────────────────
def page_admin(LANG_CODE: str, tz: ZoneInfo):
    ...
    # Top bar (tabs)
    tab_overview, tab_matches, tab_predictions, tab_backup, tab_users, tab_manual = st.tabs([
        "📊 Overview",
        "⚽ Matches",
        "👀 Predictions",
        "📦 Backup/Restore",
    ...
    # =========================================================
    # TAB: Overview (quick leaderboard preview)
    # =========================================================
    with tab_overview:
    ...
    with tab_backup:
        ...
        with bcol2:
            ...
            if up and st.button("Restore Now", key="btn_restore_now"):
                ...
                # IMPORTANT: after restoring, recompute LB to make sure it's consistent with predictions
                preds_after = load_csv(PREDICTIONS_FILE, ["User","Match","Prediction","Winner","SubmittedAt"])
                recompute_leaderboard(preds_after)

                st.success("Backup restored. App reloaded and leaderboard recomputed.")
                st.rerun()

    # =========================================================
    # TAB: Matches (Season + Reset/Test + Add/Edit + History)
    # (We'll fill the full Matches content in Part 4)
    # =========================================================
    with tab_matches:
        st.markdown("### ⚽ Matches & Season")
        ...
        with c2:
            if st.button(tr(LANG_CODE,"reset_season"), key="btn_reset_season"):
                ...
                # keep team logos + logo files on disk
        ...
        if mdf.empty:
            ...
            for idx, row in mdf.iterrows():
                ...
                with rcol[2]:
                    ...
                    nm = st.number_input(tr(LANG_CODE,"minute"), min_value=0, max_value=59, value=mins, key=f"edit_min_{idx}")
                ...
                # Save score (and move to history if result set)
                with col_actions2:
                ...
                # Delete match (also delete all predictions of that match)
                with col_actions3:
                    if st.button(tr(LANG_CODE,"delete"), key=f"btn_del_{idx}"):
                        ...
                        # remove predictions for that match
    ...
    # =========================================================
    # TAB: Predictions (View + Delete)  (filled in Part 5)
    # =========================================================
    with tab_predictions:
        # =========================================================
    # TAB: Predictions (Admin view + delete tools)
    # =========================================================
    with tab_predictions:
        ...
        if preds_view.empty:
            ...
            df_preds_full["SubmittedAtText"] = df_preds_full["SubmittedAt"].dt.strftime("%Y-%m-%d %H:%M")
            ...
            show_cols = ["User","Match","Prediction","Winner","Result","Round","Status","SubmittedAtText"]
            st.dataframe(view_df[show_cols], use_container_width=True)

        st.markdown("---")
        st.markdown("### 🧹 Delete Tools (Admin only)")

        # Always reload from disk for delete actions
        preds_now = load_csv(PREDICTIONS_FILE, ["User","Match","Prediction","Winner","SubmittedAt"])
        ...
        if preds_now.empty:
            ...
            # -------------------------------------------------
            # A) Delete a selected prediction (one row)
            # -------------------------------------------------
            st.markdown("#### 1) Delete a selected prediction (one entry)")

            # Create a stable display list (include time so it’s unique)
            preds_now["SubmittedAt_dt"] = pd.to_datetime(preds_now["SubmittedAt"], errors="coerce")
            preds_now["SubmittedAt_txt"] = preds_now["SubmittedAt_dt"].dt.strftime("%Y-%m-%d %H:%M:%S")

            preds_now["__display__"] = (
                preds_now["User"].astype(str) + " | " +
                preds_now["Match"].astype(str) + " | " +
                preds_now["Prediction"].astype(str) + " | " +
                preds_now["Winner"].astype(str) + " | " +
                preds_now["SubmittedAt_txt"].astype(str)
            )

            pick = st.selectbox(
                "Select prediction to delete",
                options=preds_now["__display__"].tolist(),
                key="pick_single_prediction"
            )

            del_c1, del_c2 = st.columns([1,2])
            with del_c1:
                confirm_single = st.checkbox("I confirm delete", key="confirm_del_single")
            with del_c2:
                if st.button("Delete Selected Prediction", key="btn_delete_selected_prediction", disabled=not confirm_single):
                    # Re-load fresh to avoid stale
                    ...
                    p["SubmittedAt_txt"] = p["SubmittedAt_dt"].dt.strftime("%Y-%m-%d %H:%M:%S")
                    p["__display__"] = (
                        p["User"].astype(str) + " | " +
                        p["Match"].astype(str) + " | " +
                        p["Prediction"].astype(str) + " | " +
                        p["Winner"].astype(str) + " | " +
                        p["SubmittedAt_txt"].astype(str)
                    )

                    before = len(p)
                    p = p[p["__display__"] != pick].drop(columns=["SubmittedAt_dt","SubmittedAt_txt","__display__"], errors="ignore")
                    after = len(p)

                    save_csv(p, PREDICTIONS_FILE)
                    recompute_leaderboard(p)

                    st.success(f"Deleted {before - after} prediction(s).")
                    st.rerun()

            st.markdown("---")

            # -------------------------------------------------
            # B) Delete ALL predictions for a selected match
            # -------------------------------------------------
            st.markdown("#### 2) Delete ALL predictions for a match")

            match_list = sorted(preds_now["Match"].dropna().astype(str).unique().tolist())
            match_pick = st.selectbox("Select match", options=match_list, key="pick_match_delete_preds")

            cA, cB = st.columns([1,2])
            with cA:
                confirm_match = st.checkbox("I confirm delete ALL for match", key="confirm_del_match_preds")
            with cB:
                if st.button("Delete Predictions for This Match", key="btn_delete_match_predictions", disabled=not confirm_match):
                    p = load_csv(PREDICTIONS_FILE, ["User","Match","Prediction","Winner","SubmittedAt"])
                    before = len(p)
                    p = p[p["Match"].astype(str) != str(match_pick)]
                    after = len(p)

                    save_csv(p, PREDICTIONS_FILE)
                    recompute_leaderboard(p)

                    st.success(f"Deleted {before - after} prediction(s) for match: {match_pick}")
                    st.rerun()
    # =========================================================
    # TAB: Users (Terminate/Delete + OTP reset) (filled in Part 6)
    # =========================================================
    with tab_users:
        # -----------------------------
# Users (Terminate / Delete) + OTP PIN Reset
# -----------------------------
with st.expander("👤 Users & OTP PIN Reset", expanded=True):
    users_df = load_users()
    ...
    # =========================================================
    # TAB: Manual Edit (Overrides)  (filled in Part 6 as well)
    # =========================================================
    with tab_manual:
        # -----------------------------
# Leaderboard Overrides (optional admin utility)
# -----------------------------
with st.expander("✏️ Manual Overrides (Predictions & Points)", expanded=False):
    ...
    if lb_current.empty:
        ...
        with util_c2:
            if st.button("Clear ALL Overrides", key="btn_clear_override_all_manual_tab"):
                ...
                st.rerun()
        with util_c3:
            if st.button("Apply Overrides → Write Leaderboard.csv", key="btn_apply_override_write_manual_tab"):
