        ...
        for member in z.infolist():
            ...
            if base in allowed:
                ...
                with open(target, "wb") as f:
                    f.write(data)
            elif name.startswith("logos/") or name.startswith("logos\\") or name.startswith(f"{os.path.basename(LOGO_DIR)}/"):
...
def apply_theme():
    st.markdown("""
    <style>
      html, body, [data-testid="stAppViewContainer"] { background: linear-gradient(180deg,#0a1f0f,#08210c); color:#eafbea; }
      .card { background: rgba(255,255,255,0.06); border: 1px solid #134e29; border-radius: 16px; padding: 12px 16px;
...
def _otp_cleanup(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    out["ExpiresAt_dt"] = pd.to_datetime(out["ExpiresAt"], errors="coerce", utc=True)
    out = out[(out["ExpiresAt_dt"].isna()) | (out["ExpiresAt_dt"] > now)]
    out = out.drop(columns=["ExpiresAt_dt"], errors="ignore")
    return out

def otp_revoke(user: str) -> None:
    df = _otp_cleanup(_load_otps())
    u = str(user).strip().casefold()
    df = df[df["User"].astype(str).str.strip().str.casefold() != u]
    _save_otps(df)
...
def save_users(df):
    ...
    for c in cols:
        if c not in out.columns:
            out[c] = None
    out["IsBanned"] = pd.to_numeric(out.get("IsBanned", 0), errors="coerce").fillna(0).astype(int)
    out[cols].to_csv(USERS_FILE, index=False)
...
def get_real_winner(match_row: pd.Series) -> str:
    ...
    if rw:
        if _is_draw_label(rw):
            return "Draw"
        return rw
    match_name = str(match_row.get("Match") or "")
    real_pair = parse_score(str(match_row.get("Result") or ""))
    if real_pair is None:
        return "Draw"
    return _winner_from_score(match_name, real_pair)

def points_for_prediction(match_row: pd.Series, pred: str, chosen_winner: str, big_game: bool) -> tuple[int, int, int]:
    match_name = str(match_row.get("Match") or "")
    real_pair = parse_score(str(match_row.get("Result") or ""))
    pred_pair = parse_score(str(pred or ""))
    ...
    pred_w = _winner_from_score(match_name, pred_pair)
    if pred_w == "Draw":
        outcome_ok = (real_w == "Draw")
    else:
        cw = str(chosen_winner or "").strip()
        if _is_draw_label(cw):
            cw = "Draw"
        outcome_ok = (real_w != "Draw" and pred_w == real_w)
...
def _load_all_matches_for_scoring() -> pd.DataFrame:
    ...
    allm = pd.concat([open_m, hist_m], ignore_index=True)
    for c in cols_open:
        if c not in allm.columns:
            allm[c] = None
    return allm[cols_open]

def _apply_overrides_to_lb(lb: pd.DataFrame) -> pd.DataFrame:
    if lb.empty:
        return lb
    o = load_overrides()
    if o.empty:
        return lb
    out = lb.merge(o, on="User", how="left", suffixes=("", "_ovr"))
    for c in ["Predictions", "Points"]:
        if f"{c}_ovr" in out.columns:
            out[c] = out[f"{c}_ovr"].where(pd.notna(out[f"{c}_ovr"]), out[c])
    out = out.drop(columns=[c for c in out.columns if c.endswith("_ovr")], errors="ignore")
    out["Predictions"] = pd.to_numeric(out["Predictions"], errors="coerce").fillna(0).astype(int)
    out["Points"] = pd.to_numeric(out["Points"], errors="coerce").fillna(0).astype(int)
    out["Exact"] = pd.to_numeric(out["Exact"], errors="coerce").fillna(0).astype(int)
    out["Outcome"] = pd.to_numeric(out["Outcome"], errors="coerce").fillna(0).astype(int)
    out = out.sort_values(["Points", "Predictions", "Exact"], ascending=[False, True, False]).reset_index(drop=True)
    return out
...
def recompute_leaderboard(predictions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if predictions_df.empty or matches_full.empty:
    ...
    for _, p in latest.iterrows():
        ...
        if mrow.empty:
            continue
        m = mrow.iloc[0]
        if parse_score(str(m.get("Result") or "")) is None:
            continue
        ...
        rows.append({"User": str(p.get("User") or ""), "Points": int(pts), "Exact": int(ex), "Outcome": int(out)})
    ...
    applied = _apply_overrides_to_lb(lb)
                        ...
                        if now_local < open_at:
                            ...
                            if not current_name:
                                ...
                                already = predictions_df[
                                    (predictions_df["User"] == current_name) & (predictions_df["Match"] == match)
                                ]
                                if not already.empty:
                                    ...
                                    typed = parse_score(val) is not None
                                    is_draw_typed = typed and (parse_score(val)[0] == parse_score(val)[1])
    ...
    with tab2:
        ...
        if lb.empty:
            ...
            for i in lb.index:
                ...
                medals.append("🥇" if r_int == 1 else "🥈" if r_int == 2 else "🥉" if r_int == 3 else "")
            lb[tr(LANG_CODE, "lb_rank")] = lb[tr(LANG_CODE, "lb_rank")].astype(str) + " " + pd.Series(medals)
...
def page_admin(LANG_CODE: str, tz: ZoneInfo):
    ...
    tab_matches, tab_predictions, tab_settings, tab_users, tab_manual = st.tabs(
        ["🏟️ Matches", "👀 Predictions", "⚙️ Settings", "👤 Users", "✏️ Manual Edit"]
    )

    with tab_matches:
        st.subheader("🏟️ Matches")

        with st.expander(tr(LANG_CODE, "add_match"), expanded=True):
            ...
            cD1, cD2, cD3, cD4, cD5 = st.columns([1, 1, 1, 1, 1])
            with cD1:
            ...
            with cD4:
                am_label = tr(LANG_CODE, "ampm_am")
                pm_label = tr(LANG_CODE, "ampm_pm")
            ...
            if st.button(tr(LANG_CODE, "btn_add_match"), key="btn_add_match"):
                if not (teamA or "").strip() or not (teamB or "").strip():
                    ...
                    row = pd.DataFrame([{
                        "Match": f"{teamA.strip()} vs {teamB.strip()}",
        ...
        if mdf.empty:
            ...
            for idx, row in mdf.iterrows():
                ...
                with r3:
                    ...
                    nm = st.number_input(tr(LANG_CODE, "minute"), min_value=0, max_value=59, value=int(mins), key=f"edit_min_{idx}")
                    nap = st.selectbox(
                        tr(LANG_CODE, "ampm"),
                        [tr(LANG_CODE, "ampm_am"), tr(LANG_CODE, "ampm_pm")],
                        index=0 if ap == tr(LANG_CODE, "ampm_am") else 1,
                        key=f"edit_ampm_{idx}",
                    )
                ...
                with s2:
                    ...
                    pr = parse_score(normalize_digits(str(row.get("Result") or "")).strip())
                    if pr is not None:
                        ra, rb = pr
                        curw = tr(LANG_CODE, "draw") if ra == rb else (new_team_a if ra > rb else new_team_b)
                    realw = st.selectbox(
                        tr(LANG_CODE, "real_winner"),
                        options=opts,
                ...
                with a1:
                    if st.button(tr(LANG_CODE, "save"), key=f"btn_save_details_{idx}"):
                        ...
                        new_match_name = f"{(new_team_a or '').strip()} vs {(new_team_b or '').strip()}"
                        ...
                        if not p.empty and (p["Match"] == row["Match"]).any():
                ...
                with a2:
                    if st.button(tr(LANG_CODE, "save") + " (Score)", key=f"btn_save_score_{idx}"):
                        ...
                        if val:
                            ...
                            current_row = mdf[mdf["Match"] == row["Match"]].drop(columns=["KO"], errors="ignore").copy()
        ...
        st.markdown("---")
        st.markdown(tr(LANG_CODE, "match_history"))
    ...
    with tab_predictions:
        ...
        all_matches_for_view = pd.concat(
            [matches_view.assign(Status="Open"), hist_view.drop(columns=["CompletedAt"], errors="ignore").assign(Status="Closed")],
            ignore_index=True,
        )

        df_preds_full = preds_view.merge(all_matches_for_view[["Match", "Kickoff", "Result", "Round", "Status"]], on="Match", how="left")
        ...
        with cF3:
            match_filter = st.selectbox(
                "Filter by match",
                options=["(All)"] + sorted(df_preds_full["Match"].dropna().unique().tolist()),
                key="preds_filter_match",
            )
        ...
        if not all_matches_list:
            ...
            delm = st.selectbox("Select match to delete all predictions", options=all_matches_list, key="del_all_match_pick")
        ...
        if preds_now.empty:
            ...
            pick = st.selectbox("Select prediction row to delete", options=choices, key="del_one_pred_pick")
...
def _apply_overrides_for_display(lb_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if o.empty:
        return lb_df
    out = lb_df.copy()
    out = out.merge(o, on="User", how="left", suffixes=("", "_ovr"))

    def _coalesce(a, b):
        return b if pd.notna(b) else a

    if "Predictions_ovr" in out.columns:
        out["Predictions"] = out.apply(lambda r: _coalesce(r.get("Predictions"), r.get("Predictions_ovr")), axis=1)
    if "Points_ovr" in out.columns:
        out["Points"] = out.apply(lambda r: _coalesce(r.get("Points"), r.get("Points_ovr")), axis=1)

    keep = ["User", "Points", "Predictions", "Exact", "Outcome"]
    for c in keep:
        if c not in out.columns:
            out[c] = 0
    out = out[keep]
...
def page_play_and_leaderboard(LANG_CODE: str, tz: ZoneInfo):
    ...
    with tab2:
        preds = load_csv(PREDICTIONS_FILE, ["User","Match","Prediction","Winner","SubmittedAt"])
        lb = recompute_leaderboard(preds)
        lb = _apply_overrides_for_display(lb)
...
def page_admin(LANG_CODE: str, tz: ZoneInfo):
    ...
    st.session_state["_admin_tabs_ready"] = True

    with tab_matches:
        st.info("Matches tab is in Part 5.")

    with tab_predictions:
        st.info("Predictions tab is in Part 5.")
    ...
    with tab_settings:
        ...
        with st.expander("🏁 Season", expanded=True):
            ...
            with c2:
                if st.button("Reset season (danger)", key="btn_reset_season_settings_tab"):
                    ...
                    for f in [
                        ...
                        SEASON_FILE, LEADERBOARD_OVERRIDES_FILE, OTP_FILE
                    ]:
                        _safe_remove(f)
            ...
            with c3:
                st.caption("Reset clears everything except the logos folder and team_logos.json")
    ...
    with tab_users:
        ...
        if users_df.empty:
            ...
            with act2:
                ...
                if st.button(del_label, key="btn_delete_user_tab_users"):
                    ...
                    if mask_keep.all():
                        ...
                        o = load_overrides()
                        if not o.empty:
                            o = o[o["User"].astype(str).str.strip().str.casefold() != str(target_name).strip().casefold()]
                            save_overrides(o)
            ...
            with act3:
                st.caption("Delete removes user + predictions + override + OTP, then recalculates leaderboard")
            ...
            with otp_c2:
                if st.button("Revoke OTP", key="btn_revoke_otp_tab_users"):
                    otp_revoke(str(target_name))
                    st.session_state.pop("last_otp_user", None)
                    st.session_state.pop("last_otp_code", None)
                    st.success("OTP revoked ✅")
            with otp_c3:
                st.caption("Valid for 10 minutes")
        ...
        if lb_current.empty:
            ...
            with e4:
                if st.button("Save Override", key="btn_save_override"):
                    o = load_overrides()
                    mask = o["User"].astype(str).str.strip().str.casefold() == str(user_pick).strip().casefold() if not o.empty else pd.Series([], dtype=bool)
                    if not o.empty and mask.any():
                        o.loc[mask, ["Predictions", "Points"]] = [int(new_preds), int(new_points)]
                    else:
            ...
            with cA:
                if st.button("Clear Override (Selected)", key="btn_clear_override_user"):
                    o = load_overrides()
                    o = o[o["User"].astype(str).str.strip().str.casefold() != str(user_pick).strip().casefold()]
                    save_overrides(o)
                    st.success("Cleared")
                    st.rerun()
            with cB:
