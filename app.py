...
def restore_from_zip(filelike) -> None:
    """
    Restore files from backup.zip into DATA_DIR.

    ✅ FIX: Always restore by BASENAME (matches.csv, users.csv, ...)
    This prevents the common bug where the ZIP contains folders
    and Streamlit extracts into a subfolder, leaving old files untouched.
    """
...
def _otp_cleanup(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # keep rows with no expiry OR expiry in future
...
def otp_generate(user: str, minutes_valid: int = 10) -> str:
    ...
    """
    otp_revoke(user)  # one active OTP per user
...
def _normalize_winner_text(w: str) -> str:
    w = (w or "").strip()
    if not w:
        return ""
    if w.casefold() in {"draw", "تعادل"}:
        return "Draw"
    return w

def get_real_winner(match_row: pd.Series) -> str:
    """
    Prefer explicit RealWinner if present,
    otherwise infer from Result using match name.
    """
    rw = _normalize_winner_text(str(match_row.get("RealWinner") or ""))
    if rw:
        return rw

    match_name = str(match_row.get("Match") or "")
    real_pair = _parse_score(str(match_row.get("Result") or ""))
...
def points_for_prediction(match_row: pd.Series, pred: str, big_game: bool, chosen_winner: str) -> tuple[int,int,int]:
    ...
    FIXED:
    - Exact score is ORDER-SENSITIVE (2-1 != 1-2)
    - Outcome uses the REAL winner vs the user's chosen winner (or inferred from prediction if empty)
    """
    match_name = str(match_row.get("Match") or "")
    real_pair = _parse_score(str(match_row.get("Result") or ""))
    pred_pair = _parse_score(str(pred or ""))
    ...
    chosen = _normalize_winner_text(str(chosen_winner or ""))
    if not chosen:
        # if user didn't pick winner (or old rows), infer from their predicted score
        chosen = _winner_from_score(match_name, pred_pair)

    pred_is_draw = (pred_pair[0] == pred_pair[1])

    if real_w == "Draw" and pred_is_draw:
        return (2 if big_game else 1, 0, 1)

    if real_w != "Draw" and chosen == real_w:
        return (2 if big_game else 1, 0, 1)
...
def recompute_leaderboard(predictions_df: pd.DataFrame) -> pd.DataFrame:
    """
    Recompute BASE leaderboard from predictions + match results (no manual overrides applied here).
    """
    ...
    preds["SubmittedAt"] = pd.to_datetime(preds.get("SubmittedAt"), errors="coerce", utc=True)

    # latest prediction per (User, Match)
    ...
    for _, p in latest.iterrows():
        ...
        # only score completed matches (Result like "2-1")
        if _parse_score(str(m.get("Result") or "")) is None:
            continue
    ...
    if role == tr(LANG_CODE, "user_login"):
        ...
        if st.button(tr(LANG_CODE, "login"), key="btn_user_login"):
            if not name_norm or not re.fullmatch(r"\d{4}", normalize_digits(pin_in or "")):
                st.error("Enter name + 4-digit PIN." if LANG_CODE=="en" else "أدخل الاسم والرقم السري (4 أرقام).")
            else:
        ...
        if st.button(tr(LANG_CODE, "register"), key="btn_register"):
            if not reg_name or not reg_name.strip():
                ...
                if exists:
                    st.error("Name already exists." if LANG_CODE=="en" else "الاسم موجود مسبقًا.")
                else:
...
# NOTE: leaderboard shown here is BASE scoring (no overrides applied yet).
# We'll apply overrides in PART 6 where leaderboard is displayed.
# ─────────────────────────────
                        ...
                        if now_local < open_at:
                            ...
                            if not current_name:
                                ...
                                if not already.empty:
                                    ...
                                    # if user typed a draw score like 1-1, force draw
...
# =========================
# ─────────────────────────────
# Admin Page (TOP TABS - everything is in the right place)
# ─────────────────────────────
def page_admin(LANG_CODE: str, tz: ZoneInfo):
            ...
            for idx, row in mdf.iterrows():
                ...
                with col_actions3:
                    if st.button(tr(LANG_CODE,"delete"), key=f"btn_del_{idx}"):
                        ...
                        # delete predictions of this match
    ...
    # =========================================================
    # TAB: Predictions (View + Delete selected prediction + Delete all predictions of match)
    # =========================================================
    with tab_predictions:
        ...
        # Delete a selected prediction (exact row)
        st.markdown("---")
    ...
    # (tab_settings, tab_users, tab_manual continue in PART 6)
    # IMPORTANT: do NOT add 'return' here; PART 6 will finish the function.
# =========================
# app.py  (PART 6/6)
# =========================

    # =========================================================
    # TAB: Settings (Season + Test Data + Backup/Restore)  ✅ FIXED restore issue
    # =========================================================
    with tab_settings:
        ...
        # 3) Backup & Restore  ✅ FIXED: restore now correctly extracts + recomputes from restored file
        with st.expander("📦 Backup & Restore", expanded=True):
            bcol1, bcol2 = st.columns([1, 1])

            with bcol1:
                if st.button("⬇️ Backup Now", key="btn_backup_now_settings_tab"):
                    # always read the latest files on disk
            ...
            with bcol2:
                ...
                if up and st.button("Restore Now", key="btn_restore_now_settings_tab"):
                    ...
                    # ✅ IMPORTANT FIX:
                    # after restore, force reload files from disk (no old cached dfs)
    ...
    # =========================================================
    # TAB: Users (Terminate/Delete + OTP generation/revoke)
    # =========================================================
    with tab_users:
        st.subheader("👤 Users")
        ...
        if lb_current.empty:
            ...
            with edit_col4:
                if st.button("Save Override", key="btn_save_override"):
                    ...
                    # ✅ IMPORTANT: also write the overridden leaderboard so USERS see it instantly
                    ...
                    st.success("Override saved (users leaderboard updated).")
                    st.rerun()
